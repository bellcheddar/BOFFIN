...
- [x] **Removed the SIFTS dependency from the core numbering path.** SIFTS states no licence, and the obvious workaround fails: RCSB is CC0 but labels its copy `provenance_source: SIFTS`, and its policy excludes data from integrated external resources. **The way out is a different source, not a different supplier** — `_pdbx_poly_seq_scheme`, `_struct_ref_seq` and `_pdbx_struct_assembly` are inside every mmCIF entry and the PDB archive is CC0. What is lost is stated rather than glossed: `_struct_ref_seq` is the *depositor's* alignment and SIFTS is EBI's curated correction of it, so this is the licence-clear path and not the more accurate one. One bug on the way would have gutted a feature: an unobserved residue does **not** have a missing author number, so reading it that way reported all 298 residues of CDK2 as observed and would have made every construct in the precedent table look fully ordered
- [x] **Settled the unstated-terms question.** Marc's decision, 2026-08-25: datasets that state no terms are used for non-commercial research. That covers SIFTS and the DTU distributions of CB513, TS115 and CASP12, and they are no longer release blockers. The factual position is on the record in `Docs/ATTRIBUTIONS.md` — silence is not a permissive licence, so this is a risk position rather than a permission, and it should be revisited before any commercial release. Every source is attributed regardless of whether a licence demands it
...
- [x] **Phase 8 (part 4): scene decks and presentation mode.** Named scenes captured from the viewer, presented full-bleed, exported as `.pml`. **Advancing stops at the end rather than wrapping** — reaching the last slide and finding the first again reads as having lost your place — and it is **not tap-to-advance**, because a tap on a molecule belongs to the molecule: in front of a room you rotate as often as you change slide. Four failures on the way, none of them in the deck: an accessibility identifier on a *stack* makes the whole stack one element and hid the controls; typing a scene name passed on iPhone and failed on iPad; a wedged simulator reported `Application failed preflight checks`; and CI's no-CDN check tripped on a *comment* in vendored Mol\* citing a SIGGRAPH paper
- [ ] **Phase 8 (rest): touch selection builder, external display and Pencil.** A visual selection builder so the language is optional, a presenter view on the device with the structure on the external display, and Pencil annotation anchored to scene index
- [x] **Phase 9 (part): clean-room `InteractionProfiler`.** PLIP is GPL v2 and is never linked, ported or consulted; what is reimplemented are the published geometric criteria, in a single tunable struct. **Protonation is the hard part and the honest answer is to say what was assumed**: most crystal structures have no hydrogens, so every profile carries a statement naming the specific choices rather than a vague disclaimer, because a reader cannot otherwise tell which contacts survive a different assumption. Where a criterion cannot be applied (hydrogen-bond angles without hydrogens, halogen angles without connectivity) it is not applied silently. Checked against CDK2's ATP site: the hinge, the glycine-rich loop, salt bridges to the triphosphate. One test tightens every cutoff and asserts strictly fewer contacts, because a criterion that is read and never applied looks exactly like one that is applied and generous
- [x] **Phase 9 (part 2): diagram, summary and CSV.** The 2D diagram is drawn natively on a **circle rather than a force-directed layout**, which would look more like the published figures and would move a residue between renders of the same structure, making two figures of one site impossible to compare. Kinds are distinguished by dash pattern as well as colour so the figure survives being printed in grey, and VoiceOver reads every contact. **The assumptions go inside the CSV**, not alongside it, and sit *above* the results in the app: a reader who stops after the numbers has to have seen what they rest on. One field was renamed after a test noticed author numbers above 300 in a 298-residue kinase — metal coordination is to the metal, so `proteinAtom` was right for four kinds out of five
- [x] **Fixed two viewer commands that were doing nothing.** The Mol\* UMD build exports **thirteen names**; `Shape`, `Symmetry` and `OrderedSet` live under `molstar.lib`, and code written against the top-level names does not throw — it evaluates to `undefined` and takes the fallback branch. `listAssemblies` returned an empty list forever, and every pick reported the *first atom of the unit* rather than the one tapped, always plausibly. Every test passed throughout, because they checked a command was **dispatched** rather than that it did anything. An empty assembly list now carries a note saying which path was taken, so "declares none" and "could not look" no longer read the same
- [ ] **Verify assembly discovery against a structure known to declare one.** Implemented and guarded, but the UI test asserting it was deleted rather than tuned: it could not pass without pinning what Mol\* reports for one file, which changes between versions
- [ ] **Phase 9 (rest): the 3D overlay.** Dashed lines in the viewer, toggleable by type. **Built twice and removed twice.** The first draft assembled a list of shapes and did nothing with them. The second used Mol\*'s measurement manager with endpoints named in PyMOL syntax, and measured **0 of 40 lines drawn**: `StructureSelectionFromScript` with `language: "pymol"` returns an empty selection in the viewer UMD build, with the state cell reporting success and no error. The counter that caught it exists because an overlay drawing nothing looks identical to one with nothing to draw. Whatever replaces it needs an explicit test that BOFFIN's atom index and Mol\*'s element index agree, rather than an assumption that they do
- [x] **Phase 10 (part): opening files and Shortcuts.** A FASTA shared from Mail, Files or AirDrop opens and analyses. The plist half is the half that is invisible when missing: without `CFBundleDocumentTypes` the share sheet never offers BOFFIN and `onOpenURL` is never called, which reads as a bug in the handler. `public.plain-text` is claimed as an **alternate**, not an owner — claiming ownership of plain text is how an app becomes the thing that opens your shopping list. Two App Intents run **without bringing the app forward**, and expose only the analysis that needs no model: a Shortcut that sometimes fails because a 67 MB asset was not downloaded is worse than one that never offered the option
- [ ] **Phase 10 (rest): extensions and sync.** Share extension, Quick Look preview, CloudKit sync, Handoff, widget and Live Activity. Each needs a new Xcode target, which `Tools/add-file-to-target.py` deliberately does not create
- [x] **Phase 11 (part): Audio Graphs.** Continuous tracks carry an `AXChartDescriptor`, so a VoiceOver user **hears** hydropathy or a ΔLLR profile rather than being read a summary of it. A track is a shape and the question a shape answers is "where does it change". Gaps stay gaps — sonifying a zero where hydropathy leaves the termini blank would be a tone where there is no measurement — only continuous tracks are sonified, and the spoken summary says *where* the extremes are rather than only what they are
- [x] **Phase 11 (part): privacy manifest and acknowledgements.** The manifest declares that BOFFIN collects **nothing** — not "nothing identifiable", nothing. Every source is credited whether or not its licence asks, and the terms column reads "None stated" in orange where that is true, because rounding an unstated licence up on an acknowledgements screen would mislead exactly where a reader is looking for the truth
- [x] **Answered whether the dataset curation was load-bearing.** Heads trained on labels taken **straight from PDB entries** — 5,012 chains, 1,476,209 residues, split by accession so the score is not memorisation — reach disorder **MCC 0.510** and three-state **accuracy 0.819**. The shipped NetSurfP-trained heads measured MCC 0.628/0.500 and Q3 0.808/0.824/0.743 on their own benchmarks, so both land inside the range. Not a like-for-like comparison and not quoted as one. The honest weakness is disorder precision at 0.396: class weighting bought recall, which for construct design is the right way round
