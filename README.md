...
- [ ] **Phase 6 (rest): linkers, construct cards and disulfides.** Linker design, construct card export with primer-ready DNA, and disulfide pairs as a hard constraint. The last one needs the structure viewer: pairs cannot be read off a sequence, so a boundary can currently separate two cysteines that pair
