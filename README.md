...
- [ ] **Phase 6 (rest): primer-ready DNA and disulfides.** Reverse translation needs a codon usage table fetched and checksummed rather than transcribed. Disulfide pairs cannot be read off a sequence, so a boundary can currently separate two cysteines that pair; that one needs the structure viewer
