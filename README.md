...
- [ ] **Phase 3 (rest): TM spans and signal peptide.** Needs its own dataset (DeepTMHMM's or TOPCONS), whose terms are unverified. Accepts when the GPCR fixture shows seven TM spans
- [ ] **Verify every dataset licence before release.** The DTU download pages state no terms at all, and `Datasets/MANIFEST.md` records that honestly rather than assuming. Blocks shipping a head trained on them, not development
...
- [ ] **Phase 5 (rest): classifier, numbering and homolog search.** Pfam classification over the pooled embedding, mapping a pasted sequence onto the bundled KLIFS and GPCRdb tables (the KLIFS pocket is discontinuous in sequence, so this needs real alignment), SIFTS mapping to PDB author numbers, and the embedding index. Tables are already fetched and bundled; the Family tab states on screen what it does not yet do
